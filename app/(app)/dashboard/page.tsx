...
const COVERAGE_BAR = Array.from({ length: 7 }, (_, i) => {
  const monday = startOfWeek(new Date(), { weekStartsOn: 1 })
  const d = addDays(monday, i)
  const dateStr = format(d, 'yyyy-MM-dd')
  const { coverage } = getDayCoverage(dateStr)
...
export default function DashboardPage() {
  ...
  const shortDays     = COVERAGE_BAR.filter(d => d.coverage !== 'full').length

  const employeeHoursMap = useMemo(() => {
    return EMPLOYEES.reduce<Record<string, number>>((acc, e) => {
      const hours = SHIFTS
        .filter(s => s.assignedEmployee?.id === e.id)
  ...
  const stats = [
    {
      ...
      sub: `${todayShifts.filter(s => s.status !== 'open').length} obsazeno`,
      icon: CalendarDays,
      color: 'text-indigo-600',
      bg: 'bg-indigo-50',
    },
    {
      label: 'Otevřené směny',
      value: openShifts.length,
      sub: 'čeká na obsazení',
      icon: Clock,
      color: 'text-amber-600',
      bg: 'bg-amber-50',
    },
    {
      label: 'Dny s nedostatkem',
      value: shortDays,
      sub: 'tento týden',
      icon: AlertTriangle,
      color: 'text-red-500',
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="max-w-md w-full text-center space-y-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link
                href="/shifts"
                className="flex flex-col items-center gap-2 p-5 rounded-2xl border-2 border-indigo-100 hover:border-indigo-400 hover:bg-indigo-50 transition-all"
              >
              ...
              <Link
                href="/employees"
                className="flex flex-col items-center gap-2 p-5 rounded-2xl border-2 border-slate-100 hover:border-indigo-200 hover:bg-slate-50 transition-all"
              >
            ...
            <p className="text-xs text-slate-400">
              Nebo prozkoumej aplikaci —{' '}
              <Link href="/open-shifts" className="text-indigo-500 hover:text-indigo-700">volné směny</Link>
            </p>
          </div>
        </div>
      </div>
                ...
                {pendingApps.map(app => (
                  <div key={app.id} className="flex items-center gap-3 px-6 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">
                        {app.shift.roleNeeded} · {app.shift.date} {app.shift.startTime}–{app.shift.endTime}
                      </p>
                    </div>
              ...
              return (
                <div key={emp.id} className="flex items-center gap-3 px-6 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">{shiftsCount} směn tento týden</p>
                  </div>
